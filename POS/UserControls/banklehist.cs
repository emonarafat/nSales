...
namespace POS
{
    public partial class banklehist : UserControl
    {
        string MyConString;
        ...
        public void show_data()
        {
            ...
            if (this.OpenConnection() == true)
            {
                ...
                foreach (DataRow tempRow_Variable in myDataTable.Rows)
                {
                    ...
                    list_ex.Items.Add(new ListViewItem(new string[] { tempRow["serial"] + "", Convert.ToDateTime(tempRow["date"]).ToShortDateString() + "", tempRow["bankname"] + "", tempRow["acnum"] + "", tempRow["subject"] + "", tempRow["catagory"] + "", tempRow["type"] + "", tempRow["amount"] + "", tempRow["issuedby"] + "" }));
                }
            }
            CloseConnection();
        }
        ...
        private void dateTimePicker2_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (this.OpenConnection() == true)
            {
                query = "SELECT * FROM banking WHERE date BETWEEN'" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "' AND '" + dateTimePicker2.Value.ToString("yyyy-MM-dd") + "'";
        ...
        private void button_remove_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem item in list_ex.Items)
            {
                if (item.Checked)
                {
                    ...
                    establish_connection();
                    string query2 = "DELETE FROM banking WHERE serial='" + value + "'";
        ...
        {
            establish_connection();
            string query2 = "DELETE FROM banking";
        ...
        private void btn_print_Click(object sender, EventArgs e)
        {
            ...
            tw.WriteLine("ID      Date       Bank Name           Accout          Subject          Catagory          Type          Amount(TK)          Issued By");
            tw.WriteLine("-------------------------------------------------------------------------------------------------------------------------------------");
            foreach (ListViewItem item in list_ex.Items)
            {
                if (list_ex.Items[i].SubItems[0].Text != "")
                {
                    tw.WriteLine(list_ex.Items[i].SubItems[0].Text + "_____" + list_ex.Items[i].SubItems[1].Text + "_____" + list_ex.Items[i].SubItems[2].Text + "_____" + list_ex.Items[i].SubItems[3].Text + "_____" + list_ex.Items[i].SubItems[4].Text + "_____" + list_ex.Items[i].SubItems[5].Text + "_____" + list_ex.Items[i].SubItems[6].Text + "_____" + list_ex.Items[i].SubItems[7].Text + "_____" + list_ex.Items[i].SubItems[8].Text);
                }
                i++;
            }
            tw.Close();
