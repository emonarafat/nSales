...
namespace POS
{
    public partial class challanhist : UserControl
    {
        string MyConString;
        ...
        public void show_data()
        {
            ...
            if (this.OpenConnection() == true)
            {
                ...
                foreach (DataRow tempRow_Variable in myDataTable.Rows)
                {
                    ...
                    list_ex.Items.Add(new ListViewItem(new string[] { Convert.ToDateTime(tempRow["date"]).ToShortDateString() + "", tempRow["tno"] + "", tempRow["carriage"] + "", tempRow["weight"] + "", tempRow["amount"] + "" }));
                }
            }
            CloseConnection();
        }
        private void challanhist_Load(object sender, EventArgs e)
        {
            show_data();
        }
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (this.OpenConnection() == true)
            {
                query = "SELECT * FROM challan WHERE date BETWEEN'" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "' AND '" + dateTimePicker2.Value.ToString("yyyy-MM-dd") + "'";
        ...
        private void btn_print_Click(object sender, EventArgs e)
        {
            ...
            tw.WriteLine("Date               Truck No.               No. of Carriage               Weight(Kg)             Amount(TK)");
            tw.WriteLine("----------------------------------------------------------------------------------------------------------");
            foreach (ListViewItem item in list_ex.Items)
            {
                if (list_ex.Items[i].SubItems[0].Text != "")
                {
                    tw.WriteLine(list_ex.Items[i].SubItems[0].Text + "_____" + list_ex.Items[i].SubItems[1].Text + "_____" + list_ex.Items[i].SubItems[2].Text + "_____" + list_ex.Items[i].SubItems[3].Text + "_____" + list_ex.Items[i].SubItems[4].Text);
                }
                i++;
            }
            tw.Close();
